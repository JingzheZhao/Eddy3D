...
## 2026-03-12 - Data Fusion and Weather Precomputation for UTCI
**Learning:** Annual simulations involving multiple sensor probes often perform redundant calculations that depend only on hourly weather data (e.g., vapor pressure `Pa`). Furthermore, sequential passes for calculation, rounding, and categorization increase memory traffic and cache misses.
**Action:** Precompute weather-only values once per simulation and pass them into optimized math kernels. Merge all data-processing stages (calculation, rounding, categorization, and aggregation) into a single `Parallel.For` loop over sensors to maximize cache efficiency and minimize thread orchestration overhead.
