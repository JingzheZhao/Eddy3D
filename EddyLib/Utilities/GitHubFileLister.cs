...
{
    /// <summary>
    /// Utility class to list files from a GitHub repository using the GitHub API.
    /// Derived from the approach in Urbano, adapted for Newtonsoft.Json.
    /// </summary>
    public sealed class GitHubFileLister : IDisposable
    {
        ...
        public GitHubFileLister(string userAgent = "eddy3d-file-lister-csharp")
        {
        ...
        public GitHubFileLister(HttpClient httpClient, string userAgent = "eddy3d-file-lister-csharp")
        {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            using (var resp = await _http.GetAsync(url, cancellationToken))
            {
                ...
                var data = JsonConvert.DeserializeObject<CommitResponse>(json)
            ...
            CancellationToken ct)
        {
            ...
            using (var resp = await _http.GetAsync(url, ct))
            {
                resp.EnsureSuccessStatusCode();
                var json = await resp.Content.ReadAsStringAsync();
                var data = JsonConvert.DeserializeObject<TreeResponse>(json)
