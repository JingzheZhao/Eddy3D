...
namespace Eddy
{
    public class ReadUTCI : GH_Component
    {
      ...
      : base("Read Results", "Read", "Read Simulation Results", "Eddy", "UTCI")
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            if (!Directory.Exists(DOM.baseWorkingDir))
            {
                return;
            }

            string file = DOM.baseWorkingDir + @"\UTCI.csv";
            ...
            for (int i = 0; i < data.GetUpperBound(0); i++)
            {
                for (int j = 0; j < data.GetUpperBound(1); j++)
                {
                    ...
                    UTCITree.Add(data[i, j], new Grasshopper.Kernel.Data.GH_Path(i));
                }
            }
